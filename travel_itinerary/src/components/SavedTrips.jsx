...
export default function SavedTrips() {
  ...
  // Only show trips the user has "liked"/saved
  const savedTrips = useMemo(
    () => trips.filter(trip => trip.liked),
    [trips]
  );

  // Handler with basic error feedback
  const handleToggle = async (tripId) => {
    try {
    ...
    <div className="pb-24">
      ...
      <div className="px-5 pt-4">
        <div className="border-b border-emerald-200 dark:border-emerald-800 pb-3 mb-4">
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm font-light italic mt-1">
            Your curated travel wishlist
          </p>
        </div>
      </div>
      ...
      <div className="mt-4 px-5">
        {savedTrips.length === 0 ? (
          ...
          >
            <div
              className="w-20 h-20 bg-gray-100 dark:bg-gray-700 rounded-full 
            ...
            <p className="text-gray-500 dark:text-gray-400 text-sm mb-6 max-w-xs mx-auto">
              Start exploring and save destinations you’re interested in for future reference
            </p>
            <motion.button
              whileHover={{ scale: 1.03 }}
              whileTap={{ scale: 0.97 }}
              className="px-5 py-3 bg-emerald-500 hover:bg-emerald-600
                         text-white font-medium rounded-xl shadow-md flex items-center
          ...
          savedTrips.map((trip, idx) => (
            ...
            >
              <div className="relative">
                ...
                <div className="absolute bottom-0 left-0 bg-gradient-to-t from-black/80 
                                via-black/60 to-transparent w-full py-3 px-4">
                  <div className="text-emerald-300 font-semibold text-xs uppercase 
                                  tracking-wider">{trip.country}</div>
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <motion.button
                      ...
                      className="flex items-center justify-center 
                                 rounded-full bg-emerald-50 dark:bg-emerald-900/30 
                                 w-10 h-10"
                    >
                      <svg xmlns="http://www.w3.org/2000/svg"
